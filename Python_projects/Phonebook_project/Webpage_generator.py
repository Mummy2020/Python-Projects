
import tkinter as tk
from tkinter import *
import webbrowser


class ParentWindow(Frame):
    def __init__(self, master):
        Frame.__init__(self, master)
        self.master.title("Web page Generator")
        self.btn = Button(self.master, text="Default HTML Page", width=30, height=2, command=self.defaultHTML)
        self.btn.grid(row=2, column=1, padx=(10,10), pady=(10, 10))
        self.btn = Button(self.master, text="Submit Custom Text", width=30, height=2, command=self.submitCustomText)
        self.btn.grid(row=2, column=2, padx=(10,10), pady=(10,10))

        self.varText = Entry(self.master)
        self.varText.grid(row=1, column=0, padx=(30,15), pady=(10,10), columnspan=3, sticky=W+E)

        self.lblText = Label(self.master,text='Enter custom text or click Default HTML page button: ', font=("Helvetica", 16), fg="black", bg="lightgray")
        self.lblText.grid(row=0, column=0,padx=(30,0),pady=(30,0))

    def defaultHTML(self):
        htmlText = "Stay tuned for our amazing summer sale!"
        htmlFile = open("index.html", "w")
        htmlContent = "<html>\n<body>\n<h1>" + htmlText + "</h1>\n</body>\n</html>"
        htmlFile.write(htmlContent)
        htmlFile.close()
        webbrowser.open_new_tab("index.html")

    def submitCustomText(self):
        htmlText = self.varText.get()
        htmlFile = open("index.html", "w")
        htmlContent = "<html>\n<body>\n<h1>" + htmlText + "</h1>\n</body>\n</html>"
        htmlFile.write(htmlContent)
        htmlFile.close()
        webbrowser.open_new_tab("index.html")
    

    

       

if __name__ == "__main__":
    root = tk.Tk()
    App = ParentWindow(root)
    root.mainloop()
